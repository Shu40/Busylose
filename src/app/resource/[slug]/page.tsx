          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="space-y-4 max-w-2xl">
              <div className="flex items-center gap-x-3">
                ...
                <span className="flex items-center gap-x-1 text-slate-400 text-xs font-bold">
                  ...
                  Live since {new Date(resource.uploadDate).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })}
                </span>
              </div>
            ...
            <div className="flex flex-col gap-4 min-w-[240px]">
              <a 
                ...
                onClick={async () => {
                   "use server";
                   await trackDownload(resource._id.toString());
                }}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800 text-center">
                  <div className="flex items-center justify-center gap-x-1 text-sky-600 mb-1">
                    ...
                    <span className="text-sm font-black">{resource.views || 0}</span>
                  </div>
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800 text-center">
                  <div className="flex items-center justify-center gap-x-1 text-green-600 mb-1">
                    ...
                    <span className="text-sm font-black">{resource.downloads || 0}</span>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <section className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-3xl p-10 text-white shadow-2xl relative overflow-hidden">
            <div className="relative z-10">
              ...
              <p className="text-slate-300 mb-8 leading-relaxed max-w-xl">
                This tool was contributed by <span className="text-white font-bold">{resource.owner}</span>. 
              ...
              <div className="flex flex-wrap gap-4">
                <div className="flex items-center gap-x-3 bg-white/10 px-5 py-3 rounded-2xl border border-white/10 backdrop-blur-md">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-white">{resource.owner}</p>
                  </div>
                </div>
                <div className="flex items-center gap-x-3 bg-white/10 px-5 py-3 rounded-2xl border border-white/10 backdrop-blur-md">
                   ...
                   <div>
                    ...
                    <p className="font-bold text-white truncate max-w-[150px]">{resource.contactEmail || "Protected"}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between py-2 border-b border-slate-50 dark:border-slate-800">
                ...
                <span className="text-green-500 font-black">{resource.uptime || "100%"}</span>
              </div>
              <div className="flex items-center justify-between py-2 border-b border-slate-50 dark:border-slate-800">
                <span className="text-slate-400 text-xs font-bold uppercase tracking-widest">Released</span>
                <span className="text-slate-700 dark:text-slate-300 font-black">{new Date(resource.uploadDate).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })}</span>
              </div>
              <div className="flex items-center justify-between py-2 border-b border-slate-50 dark:border-slate-800">
                <span className="text-slate-400 text-xs font-bold uppercase tracking-widest">Category</span>
                <span className="text-sky-600 font-black uppercase tracking-tighter">{resource.category}</span>
              </div>
            </div>
          ...
          >
            ...
            <p className="text-sky-700/70 dark:text-sky-500 group-hover:text-sky-100 text-sm font-medium">See what others are saying about {resource.title} and share your feedback.</p>
          </Link>
        </div>
      </div>
    </div>
