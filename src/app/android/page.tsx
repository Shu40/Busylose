...
export const metadata: Metadata = {
  title: "Android APKs",
  description: "Download verified Android APKs and tools on BusyLoss.",
};

export default async function AndroidPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  const session = await getServerSession(authOptions);
  
  await dbConnect();
  const { q } = await searchParams;
  const query = q || "";
  const filter: any = { category: "Android" };
  if (query) {
    filter.$or = [
  ...
  return (
    <div className="p-6 space-y-6">
      <CategoryHeader 
        title="Android Apps"
        description="Premium APKs and utility tools optimized for power users and developers."
        iconName="Smartphone"
        color="orange"
