...
export const metadata: Metadata = {
  title: "Linux Tools",
  description: "Browse high-quality Linux tools and scripts on BusyLoss.",
};

export default async function LinuxPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  const session = await getServerSession(authOptions);
  
  await dbConnect();
  const { q } = await searchParams;
  const query = q || "";
  const filter: any = { category: "Linux" };
  if (query) {
    filter.$or = [
  ...
  return (
    <div className="p-6 space-y-6">
      <CategoryHeader 
        title="Linux Tools"
        description="Open-source software, terminal utilities and scripts for professional Linux environments."
        iconName="Terminal"
        color="green"
